...
prev: /Special/Polynomials/
next: /Special/Automata/
---
...
In the following, fix a product rule `P` that is special.
The main result of this section is that the resulting `P`-product
endows the set of series with the structure of an associative commutative algebra.
...
We begin by showing that the `P`-product is associative, commutative, and distributive over addition, and compatible with scalar multiplication.
The additional size parameter allows Agda to verify that the definitions are productive.
...
The proof of the properties above is by coinduction,
relying on the fact that the semantics of terms is invariant under term equivalence.
...
Semantic invariance is in turn coinductively proved using the series properties above.
This circularity is well-founded because the size parameter decreases.
...
We now put together all the properties above.
